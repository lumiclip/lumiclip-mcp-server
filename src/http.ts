...
app.get("/health", (_req, res) => {
  res.json({ status: "ok", service: "lumiclip-mcp-server" });
});

app.get("/.well-known/mcp/server-card.json", (_req, res) => {
  res.json({
    serverInfo: { name: "lumiclip", version: "3.0.0" },
    authentication: {
      required: true,
      schemes: ["bearer"],
      description:
        "Pass your Lumiclip API key as: Authorization: Bearer sk_live_...",
      get_api_key: "https://app.lumiclip.ai/developers",
    },
    tools: [
      {
        name: "generate_clips",
...
app.post("/mcp", async (req, res) => {
  ...
  if (!authHeader || !authHeader.startsWith("Bearer ")) {
    res.status(401).json({
      error: "Missing or invalid API key. Pass your Lumiclip API key as: Authorization: Bearer sk_live_...",
      docs: "https://app.lumiclip.ai/developers",
    });
    return;
  }
...
app.get("/", (_req, res) => {
  res.json({
    name: "Lumiclip MCP Server",
    description: "Turn YouTube videos into short-form clips from any AI assistant",
    docs: "https://github.com/lumiclip/lumiclip-mcp-server",
    mcp_endpoint: "/mcp",
    auth: "Bearer <your-lumiclip-api-key>",
    get_api_key: "https://app.lumiclip.ai/developers",
  });
});

app.listen(PORT, "0.0.0.0", () => {
  console.log(`Lumiclip MCP server (Streamable HTTP) listening on port ${PORT}`);
});
