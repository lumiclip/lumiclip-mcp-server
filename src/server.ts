...
export function createServer(config: ApiConfig): McpServer {
  ...
  server.prompt(
    "check-credits",
    "Check remaining Lumiclip credits and plan details",
    () => ({
      messages: [
        {
          role: "user" as const,
          content: {
            type: "text" as const,
            text: "Check my Lumiclip credit balance and plan details using check_usage.",
          },
        },
      ],
    })
